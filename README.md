...
Open it in a current desktop browser.
Choose a local text or Markdown document.
Select a paragraph-detection method.
Select an HTML structure.
Hover over or focus an individual displayed segment using the preferred accessibility tool.
If the reading behaviour is not ideal, try another HTML structure or paragraph-detection method.
Editor variant
